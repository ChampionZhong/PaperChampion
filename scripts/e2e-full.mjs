...
(async () => {
  ...
  logo.includes("PaperChampion") ? pass("Logo 存在") : fail("Logo 缺失");
  ...
  if (chatInput) {
    ...
    if (sendBtn) {
      ...
      const hasReply = chatBody.includes("PaperChampion") || chatBody.includes("论文") || chatBody.length > 500;
  ...
  papersBody.includes("搜索导入") ? pass("侧栏: 搜索导入") : fail("侧栏缺失: 搜索导入");
