...
class LLMClient:
    ...
    def model_for_stage(self, stage: str) -> str:
        """Return the active configured model for a pipeline stage."""
        cfg = self._config()
        if stage == "fallback":
            return cfg.model_fallback
        return self._resolve_model(stage, None, cfg)
    ...
    def _pseudo_summary(
        ...
        # For research_tag stage, return parseable JSON so classifier gets ["Other"]
        if stage == "research_tag":
            pseudo = '{"tags": ["Other"]}'
        else:
            pseudo = (
