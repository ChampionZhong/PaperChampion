...
from sqlalchemy import Select, bindparam, delete, func, or_, select, text
...
class PaperRepository:
    ...
    def list_paginated(
        ...
        if category_ids:
            # Filter by arXiv categories in metadata.categories (SQLite json_each)
            cat_filter = text(
                "EXISTS (SELECT 1 FROM json_each(papers.metadata, '$.categories') "
                "WHERE json_each.value IN :cats)"
            ).bindparams(bindparam("cats", expanding=True))
            filters.append(cat_filter)
            exec_params = {"cats": tuple(category_ids)}

        if research_tags:
            # Filter by research_tags in metadata (LLM-classified labels)
            rt_filter = text(
                "EXISTS (SELECT 1 FROM json_each(papers.metadata, '$.research_tags') "
                "WHERE json_each.value IN :tags)"
            ).bindparams(bindparam("tags", expanding=True))
            filters.append(rt_filter)
            exec_params = exec_params or {}
            exec_params["tags"] = tuple(research_tags)
        ...
        total = (
            self.session.execute(count_q, exec_params or {}).scalar() or 0
        )
        offset = (max(1, page) - 1) * page_size
        _SORT_COLS = {
    ...
    def delete_paper(self, paper_id: UUID) -> None:
        """Delete a paper and cascade to related records."""
        paper = self.session.get(Paper, str(paper_id))
    ...
    def set_research_tags(self, paper_id: UUID | str, tags: list[str]) -> None:
        """Update paper metadata with research_tags (LLM-classified labels)."""
        pid = UUID(str(paper_id)) if isinstance(paper_id, str) else paper_id
        paper = self.get_by_id(pid)
        meta = dict(paper.metadata_json or {})
        meta["research_tags"] = tags
...
class TopicRepository:
    ...
    def list_topics(
        ...
        if category_only:
            q = q.where(
                or_(
                    TopicSubscription.category_id.isnot(None),
                    TopicSubscription.category_ids.isnot(None),
                )
            )
        return list(self.session.execute(q).scalars())
    ...
    def upsert_topic(
        ...
        *,
        ...
        if found:
            ...
            found.category_ids = category_ids
            ...
            found.date_filter_days = max(1, min(7, date_filter_days))
            found.date_range_start = date_range_start
            found.date_range_end = date_range_end
            found.fetch_mode = fetch_mode
        ...
        topic = TopicSubscription(
            ...
            date_filter_days=max(1, min(7, date_filter_days)),
    ...
    def update_topic(
        ...
        *,
        ...
        if category_ids is not None:
            topic.category_ids = category_ids
        ...
        if date_filter_days is not None:
            topic.date_filter_days = max(1, min(7, date_filter_days))
            # When using relative date (date_filter_days), clear absolute range
            topic.date_range_start = None
            topic.date_range_end = None
        if date_range_start is not None:
            topic.date_range_start = date_range_start
        if date_range_end is not None:
            topic.date_range_end = date_range_end
        if fetch_mode is not None:
            topic.fetch_mode = fetch_mode
...
class TopicFetchRunRepository:
    ...
    def list_by_topic(self, topic_id: str, limit: int = 50) -> list[TopicFetchRun]:
        q = (
            select(TopicFetchRun)
            .where(TopicFetchRun.topic_id == topic_id)
            .order_by(TopicFetchRun.run_at.desc())
            .limit(limit)
        )
        return list(self.session.execute(q).scalars())

    def get_by_id(self, run_id: str) -> TopicFetchRun | None:
        return self.session.get(TopicFetchRun, run_id)

    def delete(self, run_id: str) -> bool:
        """Delete a fetch run by ID. Returns True if deleted."""
        run = self.session.get(TopicFetchRun, run_id)
        if run is not None:
