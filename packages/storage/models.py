...
class TopicSubscription(Base):
    ...
    # Multi-category: comma-separated e.g. "cs.LG,cs.CV" (use when >1 category)
    category_ids: Mapped[str | None] = mapped_column(String(512), nullable=True)
    ...
    enable_date_filter: Mapped[bool] = mapped_column(nullable=False, default=False)
    date_filter_days: Mapped[int] = mapped_column(nullable=False, default=7)
    # Absolute date range for fetch_mode=date (max 7 days from today)
    ...
    # fetch_mode: "quantity" | "date"
    fetch_mode: Mapped[str] = mapped_column(String(20), nullable=False, default="quantity")
    created_at: Mapped[datetime] = mapped_column(DateTime, default=_utcnow, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(
        DateTime, default=_utcnow, onupdate=_utcnow, nullable=False
    )
...
class TopicFetchRun(Base):
    """每日论文抓取运行记录 - 不自动入库，用户手动勾选入库"""
    ...
    papers_json: Mapped[list] = mapped_column(
...
class EmailConfig(Base):
    ...
    sender_name: Mapped[str] = mapped_column(String(128), nullable=False, default="PaperChampion")
