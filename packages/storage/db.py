...
def _safe_add_column_nullable(conn, table: str, column: str, col_type: str) -> None:
    """安全添加可空列（已存在则跳过）"""
    try:
...
def run_migrations() -> None:
    ...
    with engine.connect() as conn:
        # If database is empty (e.g. after manual delete), create all tables first
        inspector = inspect(engine)
        tables = inspector.get_table_names()
        if "topic_subscriptions" not in tables:
            logger.info("Database appears empty, creating all tables from models...")
        ...
        _safe_add_column_nullable(conn, "topic_subscriptions", "category_id", "VARCHAR(32)")
        _safe_add_column_nullable(conn, "topic_subscriptions", "category_ids", "VARCHAR(512)")
        _safe_add_column(conn, "topic_subscriptions", "fetch_mode", "VARCHAR(20)", "'quantity'")
        _safe_add_column_nullable(conn, "topic_subscriptions", "date_range_start", "DATE")
        _safe_add_column_nullable(conn, "topic_subscriptions", "date_range_end", "DATE")
        # topic_fetch_runs: run history for daily papers (no auto-ingest)
        try:
            conn.execute(
                text("""
                CREATE TABLE IF NOT EXISTS topic_fetch_runs (
                    id VARCHAR(36) PRIMARY KEY,
                    topic_id VARCHAR(36) NOT NULL
                        REFERENCES topic_subscriptions(id) ON DELETE CASCADE,
                    run_at DATETIME NOT NULL,
                    query VARCHAR(1024) NOT NULL,
                    fetch_mode VARCHAR(20) NOT NULL DEFAULT 'quantity',
                    date_range_start DATE,
                    date_range_end DATE,
                    status VARCHAR(20) NOT NULL DEFAULT 'completed',
                    paper_count INTEGER NOT NULL DEFAULT 0,
                    papers_json JSON NOT NULL DEFAULT '[]',
                    error_message TEXT
                )
                """)
            )
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS ix_topic_fetch_runs_topic_id "
                    "ON topic_fetch_runs (topic_id)"
                )
            )
            conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS ix_topic_fetch_runs_run_at "
                    "ON topic_fetch_runs (run_at)"
                )
            )
            conn.commit()
        except Exception:
            ...
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS action_papers (
                    ...
                    action_id VARCHAR(36) NOT NULL
                        REFERENCES collection_actions(id) ON DELETE CASCADE,
                    paper_id VARCHAR(36) NOT NULL REFERENCES papers(id) ON DELETE CASCADE,
