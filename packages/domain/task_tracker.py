...
class TaskInfo:
    ...
    error: str | None = None
    result: Any = None
    topic_id: str | None = None  # For fetch tasks: topic being fetched
...
class TaskTracker:
    ...
    def submit(
        ...
        **kwargs: Any,
        ...
        self.start(task_id, task_type, title, total=total, topic_id=topic_id)

        def _run():
            def update_progress(msg: str, cur: int, tot: int):
                self.update(task_id, cur, msg, total=tot or total)
            ...
            try:
                ...
                success, error = self._outcome_from_result(result)
                with self._lock:
                    task = self._tasks.get(task_id)
                    if task:
                        task.result = result
                self.finish(task_id, success=success, error=error)
                if success:
                    logger.info("Task %s completed: %s", task_id, title)
                else:
                    logger.warning("Task %s failed: %s - %s", task_id, title, error)
    ...
    def _outcome_from_result(self, result: Any) -> tuple[bool, str | None]:
        ...
        raw_error = result.get("error") or result.get("error_message") or result.get("message")
        error = str(raw_error or "Task failed")[:200]
        return False, error
