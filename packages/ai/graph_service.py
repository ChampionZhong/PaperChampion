...
class GraphService:
        ...
        with session_scope() as session:
            ...
            for i, p in enumerate(valid_papers):
                ...
                points.append({
                    ...
                    "read_status": p.read_status.value if p.read_status else "unread",
                    "topics": topics,
                    "topic": topics[0] if topics else "搜索导入",
                    "arxiv_id": p.arxiv_id,
    ...
    def weekly_evolution(
        ...
        llm_result = self.llm.complete_json(
            prompt,
            stage="rag",
            model_override=self.llm.model_for_stage("rag"),
        )
        self.llm.trace_result(
            llm_result,
            stage="graph_evolution",
            prompt_digest=f"evolution:{keyword}",
        )
    ...
    def detect_research_gaps(
        ...
        result = self.llm.complete_json(
            prompt,
            stage="deep",
            model_override=self.llm.model_for_stage("deep"),
            max_tokens=8192,
        )
        self.llm.trace_result(
            result,
            stage="graph_research_gaps",
            prompt_digest=f"gaps:{keyword}",
        )
    ...
    def topic_wiki(
        ...
        self.llm.trace_result(
            outline_result,
            stage="wiki_outline",
            prompt_digest=f"outline:{keyword}",
        )
