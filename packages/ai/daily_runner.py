...
from packages.storage.repositories import (
    PaperRepository,
    TopicRepository,
    TopicFetchRunRepository,
)

logger = logging.getLogger(__name__)


def run_topic_fetch(topic_id: str, progress_callback=None) -> dict:
    """
    每日论文：仅抓取，不入库。结果存入运行历史，用户手动勾选入库。

    Returns:
        dict: {run_id, topic_id, topic_name, status, paper_count, error?}
    """
    ...
    with session_scope() as session:
        ...
        fetch_mode = getattr(topic, "fetch_mode", "quantity")
        ...
        try:
            papers = pipelines.fetch_arxiv_only(
                ...
                sort_by="lastUpdatedDate" if fetch_mode == "date" else "submittedDate",
...
def run_daily_ingest() -> dict:
    """遍历所有 enabled 主题执行抓取。分类订阅用 fetch（不入库），关键词订阅用 ingest（自动入库）"""
    with session_scope() as session:
        ...
        category_ids = {
            ...
            if getattr(t, "category_id", None) or getattr(t, "category_ids", None)
        }

    results = []
    for tid in topic_ids:
        if tid in category_ids:
            results.append(run_topic_fetch(tid))
        else:
