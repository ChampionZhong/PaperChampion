...
def build_deep_prompt(
    ...
    return (
        "你是审稿专家。请基于论文正文输出严格 JSON 对象，"
        "不要输出 markdown，不要复制字段说明或占位文本。\n"
        "JSON 字段必须为：\n"
        "- method_summary: 用中文具体说明论文方法、数据构造、训练或分析流程。\n"
        "- experiments_summary: 用中文具体说明实验设置、模型/数据集、指标和主要结果。\n"
        "- ablation_summary: 用中文具体说明消融、对比或缺失消融带来的审稿判断。\n"
        "- reviewer_risks: 中文字符串数组，列出具体审稿风险。\n"
        "要求：所有字段必须包含论文特定信息，不能使用泛泛模板词。\n"
        f"论文标题: {title}\n"
