...
const DASHBOARD_TOOL = { to: "/dashboard", icon: Activity, label: "看板" };
...
const TOOLBOX_PATHS = TOOLBOX_TOOLS.map((t) => t.to);
...
export default function Sidebar() {
  ...
  const [toolboxOpen, setToolboxOpen] = useState(() =>
    TOOLBOX_PATHS.some((p) => location.pathname.startsWith(p))
  );

  useEffect(() => {
    if (TOOLBOX_PATHS.some((p) => location.pathname.startsWith(p))) {
      setToolboxOpen(true);
    }
  }, [location.pathname]);
          ...
          <div className="flex flex-col gap-1">
            {DIRECT_TOOLS.map((tool) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    "relative flex items-center gap-2 rounded-xl px-3 py-2.5 transition-all",
                    isActive
            ...
            <div className="flex flex-col gap-0.5">
              ...
              {toolboxOpen && (
                <div className="ml-4 flex flex-col gap-0.5 border-l-2 border-border pl-2">
                  {TOOLBOX_TOOLS.map((tool) => (
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        cn(
                          "relative flex items-center gap-2 rounded-lg px-2.5 py-2 transition-all",
                          isActive
                            ? "bg-primary-light text-primary shadow-sm"
                            : "text-ink-tertiary hover:bg-hover hover:text-ink-secondary",
                        )
            ...
            >
              <DASHBOARD_TOOL.icon className="h-4.5 w-4.5 shrink-0" />
              <span className="text-[13px] font-medium leading-tight truncate">
                {DASHBOARD_TOOL.label}
              </span>
            </NavLink>
          </div>
        </div>
