...
export const topicApi = {
  list: (enabledOnly = false, categoryOnly = false) =>
    get<{ items: Topic[] }>(
      `/topics?enabled_only=${enabledOnly}&category_only=${categoryOnly}`
    ),
  getArxivCategories: () =>
    get<ArxivTaxonomy>("/topics/arxiv-categories"),
  ...
  listRuns: (topicId: string, limit = 50) =>
    get<{ items: TopicRunSummary[] }>(`/topics/${topicId}/runs?limit=${limit}`),
  getRun: (runId: string) =>
    get<TopicRunDetail>(`/topic-runs/${runId}`),
  ingestRunPapers: (runId: string, arxivIds: string[]) =>
    post<{ ingested: number; papers: { id: string; title: string; arxiv_id: string; publication_date: string | null }[] }>(
      `/topic-runs/${runId}/ingest`,
      { arxiv_ids: arxivIds }
    ),
  deleteRun: (runId: string) =>
    del<{ deleted: string }>(`/topic-runs/${runId}`),
...
export const paperApi = {
  latest: (opts: {
    ...
    if (opts.categories?.length) params.append("categories", opts.categories.join(","));
    if (opts.researchTags?.length) params.append("research_tags", opts.researchTags.join(","));
  ...
  researchTags: () => get<{ tags: string[] }>("/papers/research-tags"),
  classifyResearchTags: (paperIds: string[]) =>
    post<{ classified: number }>("/papers/classify-research-tags", { paper_ids: paperIds }),
...
export interface FigureAnalysisResponse {
  task_id?: string;
  status?: string;
  message?: string;
  paper_id?: string;
...
export interface ArxivSearchResult {
  papers: { arxiv_id: string; title: string; abstract?: string; publication_date?: string | null; metadata?: Record<string, unknown>; already_ingested?: boolean }[];
  query: string;
}

export const ingestApi = {
  searchArxiv: (query: string, maxResults = 20, sortBy = "submittedDate") => {
    const params = new URLSearchParams({ query, max_results: String(maxResults), sort_by: sortBy });
    return get<ArxivSearchResult>(`/ingest/arxiv/search?${params}`);
  },
  ingestSelected: (arxivIds: string[], topicId?: string) =>
    post<IngestResult>("/ingest/arxiv/selected", { arxiv_ids: arxivIds, topic_id: topicId || null }),
