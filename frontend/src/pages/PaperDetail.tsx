...
import { paperApi, pipelineApi, tasksApi, type FigureAnalysisItem, type FigureAnalysisResponse } from "@/services/api";
import {
  isTaskStartResponse,
  ...
  planAutoAnalysis,
  type AutoAnalysisTask,
} from "@/lib/paperDetailRuntime";
import type { Paper, SkimReport, DeepDiveReport, ReasoningChainResult } from "@/types";
import {
  ArrowLeft, ExternalLink, Eye, BookOpen, Cpu, AlertTriangle,
...
const DOWNLOAD_STAGES = ["准备 PDF 下载...", "从 arXiv 获取 PDF...", "写入本地存储..."];

const AUTO_TASK_LABELS: Record<AutoAnalysisTask, string> = {
  downloadPdf: "PDF 下载",
  skim: "粗读",
  deep: "精读",
  figures: "图表",
...
function PipelineProgress({
  ...
  type: "download" | "skim" | "deep" | "figure" | "reasoning" | "embed";
  onCancel?: () => void;
}) {
...
async function waitForTaskResult<T>(
  taskId: string,
  timeoutMs = 180000,
  intervalMs = 1500,
): Promise<T> {
...
export default function PaperDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([p, figRes]) => {
        ...
        setFigures(normalizeFigureItems(figRes.items));
  ...
  const refreshPaperDetail = useCallback(async () => {
    if (!id) return null;
    const updated = await paperApi.detail(id);
    setPaper(updated);
    setEmbedDone(updated.has_embedding ?? false);
    if (updated.skim_report) setSavedSkim(updated.skim_report);
    if (updated.deep_report) {
      setSavedDeep(updated.deep_report);
      setDeepReport(null);
    }
  ...
  const runDownloadPdf = useCallback(async (opts?: { openReader?: boolean }) => {
    if (!id || !paper) throw new Error("Paper is not loaded");
    if (!paper.arxiv_id || paper.arxiv_id.startsWith("ss-")) {
      throw new Error("该论文没有有效的 arXiv ID，无法下载 PDF");
    }
    await paperApi.downloadPdf(id);
    const updated = await refreshPaperDetail();
    if (opts?.openReader && updated?.pdf_path) setReaderOpen(true);
    return updated;
  }, [id, paper, refreshPaperDetail]);

  const handleDownloadPdf = async () => {
    setPdfDownloading(true);
    try {
      toast("info", "正在下载 PDF...");
      const updated = await runDownloadPdf({ openReader: true });
      toast("success", updated?.pdf_path ? "PDF 已就绪" : "PDF 下载完成");
    } catch (e) {
  ...
  const runFigureAnalysis = useCallback(async () => {
    if (!id) return [];
    const started = await paperApi.analyzeFigures(id, 10);
    const result = isTaskStartResponse(started)
      ? await waitForTaskResult<FigureAnalysisResponse>(started.task_id)
      : started;
    return normalizeFigureItems(result.items);
  }, [id]);
  ...
  const handleAnalyzeFigures = async () => {
    ...
    try {
      const items = await runFigureAnalysis();
      setFigures(items);
      toast("success", `解读完成，共 ${items.length} 张图表`);
    } catch (exc) {
      toast("error", exc instanceof Error ? exc.message : "图表分析失败");
    } finally { setFiguresAnalyzing(false); }
  };

  const [autoAnalyzing, setAutoAnalyzing] = useState(false);
  const [autoStage, setAutoStage] = useState("");

  const setAutoTaskLoading = (task: AutoAnalysisTask, isLoading: boolean) => {
    if (task === "downloadPdf") setPdfDownloading(isLoading);
    if (task === "skim") setSkimLoading(isLoading);
    if (task === "deep") setDeepLoading(isLoading);
    if (task === "figures") setFiguresAnalyzing(isLoading);
    if (task === "reasoning") setReasoningLoading(isLoading);
    if (task === "embed") setEmbedLoading(isLoading);
  };

  const handleAutoAnalyze = async () => {
    if (!id || !paper) return;
    const plan = planAutoAnalysis({
      hasPdf: Boolean(paper.pdf_path),
      canDownloadPdf: Boolean(paper.arxiv_id && !paper.arxiv_id.startsWith("ss-")),
      hasSkim,
      hasDeep,
      hasFigures,
      hasReasoning,
      hasEmbedding: embedDone === true || paper.has_embedding === true,
    });
    const scheduledCount = plan.ready.length + plan.afterPdf.length;
    if (scheduledCount === 0 && plan.blocked.length === 0) {
      toast("success", "所有 Pipeline 已完成");
      return;
    }
    setAutoAnalyzing(true);
    const results: AutoTaskResult[] = [];

    const runAutoTask = async (task: AutoAnalysisTask): Promise<AutoTaskResult> => {
      setAutoTaskLoading(task, true);
      try {
        if (task === "downloadPdf") {
          ...
          setReportTab("figures");
          setFigures(await runFigureAnalysis());
        ...
        setAutoTaskLoading(task, false);
      }
    };

    try {
      if (plan.ready.length > 0) {
        setAutoStage(plan.afterPdf.length > 0
          ? "下载 PDF，同时执行粗读和嵌入..."
          : "并行执行缺失 Pipeline...");
        results.push(...await Promise.all(plan.ready.map(runAutoTask)));
      }

      const pdfDownloadFailed = results.some((result) => (
        result.task === "downloadPdf" && !result.ok
      ));
      if (plan.afterPdf.length > 0 && !pdfDownloadFailed) {
        setAutoStage("PDF 已就绪，并行执行精读、图表和推理链...");
        results.push(...await Promise.all(plan.afterPdf.map(runAutoTask)));
      } else if (plan.afterPdf.length > 0) {
        plan.afterPdf.forEach((task) => {
          results.push({ task, ok: false, error: "PDF 下载失败，已跳过" });
        });
      }

      await refreshPaperDetail().catch(() => null);
      const failed = results.filter((result) => !result.ok);
      const blocked = plan.blocked.map((task) => AUTO_TASK_LABELS[task]);
      if (failed.length === 0 && blocked.length === 0) {
        toast("success", "所有 Pipeline 已完成");
      } else {
        const failedLabels = failed.map((result) => AUTO_TASK_LABELS[result.task]);
        toast("error", `部分 Pipeline 未完成：${[...failedLabels, ...blocked].join("、")}`);
      }
  ...
  const savedSkimReport = parseSavedSkimReport(savedSkim);
  const savedDeepReport = parseSavedDeepDiveReport(savedDeep?.deep_dive_md);
  const activeSkimReport = skimReport ?? savedSkimReport;
  const activeDeepReport = deepReport ?? savedDeepReport;
  const hasSkim = !!activeSkimReport;
  const hasDeep = !!activeDeepReport;
  const figureItems = normalizeFigureItems(figures);
  const hasFigures = figureItems.length > 0;
  const hasReasoning = !!reasoning;
  const hasSimilar = similarIds.length > 0;
  const hasPdf = Boolean(paper.pdf_path);
  const canDownloadPdf = Boolean(paper.arxiv_id && !paper.arxiv_id.startsWith("ss-"));
  const canReadPdf = hasPdf || canDownloadPdf;
  const hasEmbedding = embedDone === true || paper.has_embedding === true;
  const deepReviewerRisks = normalizeReviewerRisks(activeDeepReport?.reviewer_risks);
  ...
  const anyPipelineRunning = pdfDownloading || skimLoading || deepLoading || figuresAnalyzing || reasoningLoading || embedLoading;
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      <div className="space-y-3">
        ...
        {/* 功能按键：第一行 下载 / 粗读 / 精读 / 推理链，第二行 阅读 / 嵌入 / 相似 / 图表 */}
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
          <button
            onClick={handleDownloadPdf}
            disabled={!canDownloadPdf || pdfDownloading}
            className={`${ACTION_BUTTON_CLASS} ${ACTION_TONE_CLASSES.download}`}
            title={!canDownloadPdf ? "该论文没有有效的 arXiv ID，无法下载 PDF" : "下载 PDF 到本地存储"}
          >
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${ACTION_ICON_CLASSES.download}`}>
              {pdfDownloading ? <Loader2 className="h-5 w-5 animate-spin" /> : hasPdf ? <Check className="h-5 w-5" /> : <Download className="h-5 w-5" />}
            </div>
            <div className="text-left">
              <p className="text-sm font-semibold text-ink">下载 PDF</p>
              <p className="text-xs text-ink-tertiary">
                {pdfDownloading ? "下载中..." : hasPdf ? "已下载" : "从 arXiv 获取"}
              </p>
            </div>
          </button>
          <button
            ...
            className={`${ACTION_BUTTON_CLASS} ${ACTION_TONE_CLASSES.skim}`}
          >
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${ACTION_ICON_CLASSES.skim}`}>
              {skimLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : hasSkim ? <Check className="h-5 w-5" /> : <Eye className="h-5 w-5" />}
            </div>
            <div className="text-left">
              <p className="text-sm font-semibold text-ink">{hasSkim ? "已粗读" : "粗读"}</p>
              <p className="text-xs text-ink-tertiary">{skimLoading ? "分析中..." : "快速提取要点"}</p>
            </div>
          </button>
          <button
            onClick={handleDeep}
            disabled={deepLoading || !hasPdf}
            className={`${ACTION_BUTTON_CLASS} ${ACTION_TONE_CLASSES.deep}`}
            title={!hasPdf ? "需要先下载 PDF 才能精读" : ""}
          >
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${ACTION_ICON_CLASSES.deep}`}>
              {deepLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : hasDeep ? <Check className="h-5 w-5" /> : <BookOpen className="h-5 w-5" />}
            </div>
            <div className="text-left">
              <p className="text-sm font-semibold text-ink">{hasDeep ? "已精读" : "精读"}</p>
              <p className="text-xs text-ink-tertiary">{deepLoading ? "深度分析中..." : !hasPdf ? "无 PDF，需先下载" : "方法论 + 实验 + 风险"}</p>
            </div>
          </button>
          <button
            onClick={handleReasoning}
            disabled={reasoningLoading || !hasPdf}
            title={!hasPdf ? "需要 PDF 才能进行推理链分析" : ""}
            className={`${ACTION_BUTTON_CLASS} ${ACTION_TONE_CLASSES.reasoning}`}
          >
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${ACTION_ICON_CLASSES.reasoning}`}>
              {reasoningLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : hasReasoning ? <Check className="h-5 w-5" /> : <Brain className="h-5 w-5" />}
            </div>
            <div className="text-left">
              <p className="text-sm font-semibold text-ink">{hasReasoning ? "已推理" : "推理链"}</p>
              <p className="text-xs text-ink-tertiary">{reasoningLoading ? "分析中..." : !hasPdf ? "无 PDF，需先下载" : "分步推理评估"}</p>
            </div>
          </button>
          <button
            onClick={() => setReaderOpen(true)}
            disabled={!canReadPdf}
            className={`${ACTION_BUTTON_CLASS} ${ACTION_TONE_CLASSES.reader}`}
            title={!canReadPdf ? "该论文没有可阅读 PDF" : ""}
          >
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${ACTION_ICON_CLASSES.reader}`}>
              <FileSearch className="h-5 w-5" />
            </div>
            <div className="text-left">
              <p className="text-sm font-semibold text-ink">阅读</p>
              <p className="text-xs text-ink-tertiary">
                {hasPdf ? "PDF 阅读器（本地）" : canDownloadPdf ? "PDF 阅读器（arXiv 在线）" : "无原文"}
              </p>
            </div>
          </button>
          <button
            onClick={handleEmbed}
            disabled={embedLoading || hasEmbedding}
            className={`${ACTION_BUTTON_CLASS} ${ACTION_TONE_CLASSES.embed}`}
          >
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${ACTION_ICON_CLASSES.embed}`}>
              {embedLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : hasEmbedding ? <Check className="h-5 w-5" /> : <Cpu className="h-5 w-5" />}
            </div>
            <div className="text-left">
              <p className="text-sm font-semibold text-ink">{hasEmbedding ? "已嵌入" : "嵌入"}</p>
              <p className="text-xs text-ink-tertiary">{embedLoading ? "计算中..." : "向量检索准备"}</p>
            </div>
          </button>
          <button
            onClick={handleSimilar}
            disabled={similarLoading || !hasEmbedding}
            title={!hasEmbedding ? "请先执行向量嵌入" : ""}
            className={`${ACTION_BUTTON_CLASS} ${ACTION_TONE_CLASSES.similar}`}
          >
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${ACTION_ICON_CLASSES.similar}`}>
              {similarLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <Link2 className="h-5 w-5" />}
            </div>
            <div className="text-left">
              <p className="text-sm font-semibold text-ink">相似</p>
              <p className="text-xs text-ink-tertiary">{!hasEmbedding ? "需先嵌入" : "查找相似论文"}</p>
            </div>
          </button>
          <button
            onClick={handleAnalyzeFigures}
            disabled={figuresAnalyzing || !hasPdf}
            title={!hasPdf ? "需要先下载 PDF 才能解读图表" : ""}
            className={`${ACTION_BUTTON_CLASS} ${ACTION_TONE_CLASSES.figures}`}
          >
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${ACTION_ICON_CLASSES.figures}`}>
              {figuresAnalyzing ? <Loader2 className="h-5 w-5 animate-spin" /> : hasFigures ? <Check className="h-5 w-5" /> : <ImageIcon className="h-5 w-5" />}
            </div>
            <div className="text-left">
              <p className="text-sm font-semibold text-ink">{hasFigures ? `图表 (${figureItems.length})` : "图表"}</p>
              <p className="text-xs text-ink-tertiary">{figuresAnalyzing ? "解读中..." : !hasPdf ? "无 PDF，需先下载" : "Vision 解析图表"}</p>
            </div>
          </button>
        </div>
      </div>
      ...
      {pdfDownloading && <PipelineProgress type="download" />}
      ...
      <div className="space-y-4">
        ...
        <div className="min-h-[200px]">
                  ...
                  <div className="space-y-4">
                    <div className="rounded-xl bg-primary/5 p-4 dark:bg-primary/10">
                      <div className="flex items-start gap-2">
                        ...
                        <p className="text-sm font-medium text-ink">{activeSkimReport.one_liner}</p>
                      </div>
                    </div>
                    {activeSkimReport.innovations.length > 0 && (
                      <div>
                        ...
                        <ul className="space-y-1.5">
                          {activeSkimReport.innovations.map((item, i) => (
            ...
            <div className="animate-fade-in">
              {deepLoading ? null : activeDeepReport ? (
                <Card className="rounded-2xl border-blue-500/20">
                  ...
                  <div className="space-y-4">
                    <ReportSection icon={<FlaskConical className="h-4 w-4 text-blue-500" />} title="方法论" content={activeDeepReport.method_summary} />
                    <ReportSection icon={<Microscope className="h-4 w-4 text-success" />} title="实验结果" content={activeDeepReport.experiments_summary} />
                    <ReportSection icon={<Sparkles className="h-4 w-4 text-amber-500" />} title="消融实验" content={activeDeepReport.ablation_summary} />
                    {deepReviewerRisks.length > 0 && (
                      <div>
                        <h4 className="mb-2 flex items-center gap-1.5 text-sm font-medium text-ink">
                          <Shield className="h-4 w-4 text-red-500" /> 审稿风险
                        </h4>
                        <ul className="space-y-1.5">
                          {deepReviewerRisks.map((risk, i) => (
          ...
          {reportTab === "figures" && (
            <div className="animate-fade-in">
              {figuresAnalyzing ? null : figureItems.length > 0 ? (
                <Card className="rounded-2xl">
                  <CardHeader title="图表解读" description={`共 ${figureItems.length} 张图表`} />
                  <div className="space-y-3">
                    {figureItems.map((fig, i) => (
          ...
          {reportTab === "similar" && (
            <div className="animate-fade-in">
              {similarLoading ? null : similarIds.length > 0 ? (
                ...
                <EmptyReport icon={<Link2 className="h-8 w-8" />} label={hasEmbedding ? "点击「相似」按钮查找" : "请先执行「嵌入」，再查找相似论文"} />
              )}
            </div>
          )}
        </div>
