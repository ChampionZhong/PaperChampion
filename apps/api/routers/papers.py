...
@router.get("/papers/research-tags")
def list_research_tags() -> dict:
    """Return predefined research tags for filtering."""
    from packages.ai.research_tag_service import RESEARCH_TAGS

    return {"tags": RESEARCH_TAGS}


@router.post("/papers/classify-research-tags")
def classify_research_tags(req: dict) -> dict:
    """Classify selected papers by research tags (LLM). Returns classified count."""
    from packages.ai.research_tag_service import classify_papers_concurrent

    paper_ids = req.get("paper_ids") or []
    if not paper_ids:
        return {"classified": 0}

    # Limit batch size to avoid timeout
    paper_ids = paper_ids[:100]
    ...
    with session_scope() as session:
        ...
        for pid in paper_ids:
            try:
                ...
                items.append((str(p.id), p.title or "", p.abstract or ""))
            except ValueError:
                pass
    ...
    classify_papers_concurrent(items, max_workers=8)
    cache.invalidate("folder_stats")
    return {"classified": len(items)}
...
def latest(
    ...
    research_tags: str | None = Query(default=None),
    ...
    research_tags_list = None
    if research_tags:
        research_tags_list = [t.strip() for t in research_tags.split(",") if t.strip()]
        if not research_tags_list:
            research_tags_list = None
    ...
    with session_scope() as session:
        ...
        papers, total = repo.list_paginated(
            ...
            research_tags=research_tags_list,
...
def paper_detail(paper_id: UUID) -> dict:
    with session_scope() as session:
        ...
        return {
            ...
            "research_tags": (p.metadata_json or {}).get("research_tags", []),
