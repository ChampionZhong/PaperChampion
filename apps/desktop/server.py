"""
PaperChampion Desktop Server — PyInstaller 入口
...
logger = logging.getLogger("paperchampion.desktop")
...
def _apply_env_overrides(data_dir: Path, env_file: Path | None) -> None:
    ...
    """
    os.environ["DATABASE_URL"] = f"sqlite:///{data_dir / 'paperchampion.db'}"
    os.environ["PDF_STORAGE_ROOT"] = str(data_dir / "papers")
    os.environ["BRIEF_OUTPUT_ROOT"] = str(data_dir / "briefs")

    if env_file and env_file.is_file():
        os.environ["PAPERCHAMPION_ENV_FILE"] = str(env_file)
...
def main() -> None:
    data_dir = Path(os.environ.get("PAPERCHAMPION_DATA_DIR", "")).expanduser()
    env_file_str = os.environ.get("PAPERCHAMPION_ENV_FILE", "")
    env_file = Path(env_file_str).expanduser() if env_file_str else None

    if not data_dir or not data_dir.is_absolute():
        data_dir = Path.home() / "Library" / "Application Support" / "PaperChampion" / "data"
    ...
    logger.info("PaperChampion Desktop starting on 127.0.0.1:%d", port)
